...
creators per drop, refunds, compatibility with the earlier prototype.
...
export ledger kCommit: Map<Uint<64>, Bytes<32>>;             // dropId -> sha256(K_drop)
...
Buyer app carried over from the earlier prototype minus its payment-request and QR modules. Kept: `seal.ts`, `poller.ts`,
...
Port of `the earlier prototype/creator`. Kept unchanged: `content.ts` (AES-256-GCM, `h_content`), `attestation.ts`
and `qvl-verifier.ts` (three-step quote verification), `price.ts` (NIGHT → STAR).

Changed:
- `provision.ts`: payload per I5 (no private key, no deposit address, `price_star`, `title`).
