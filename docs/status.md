...
- [x] **2026-09-13 재배포(배달 기록 컨트랙트별 분리):** 이미지 0.3.1 `sha256:3ce11be2…` (run 34714387325), 컨트랙트 동일 `84d80ed0…`, RTMR3 `1fd901f6…2c05`, `smoke:live` 통과, 사이트 재배포. 원인: CVM 볼륨의 `dispatched.json`이 이전 컨트랙트의 구매 0·1번을 배달 완료로 기억해 새 컨트랙트의 0번 구매가 건너뛰어졌다(구매자 화면 'Waiting for the sealed key…'에서 정지). 이제 `dispatched-<contract>.json`. 재배포 뒤에는 크리에이터가 복구 파일로 콘텐츠 1을 다시 provision 해야 대기 중인 구매 0번이 배달된다.
...
- [x] CVM stop → start(같은 인스턴스) 후 RTMR3가 유지되는지 확인 — **2026-09-13 테스트: 유지되지 않는다.** `phala cvms stop` → `start`(약 40초) 뒤 RTMR3가 `1fd901f6…` → `16278d9f…`로 바뀌었다. provisioning 키(`349c03a3…`)와 볼륨(배달 blob 4개)은 유지. 키 카탈로그는 메모리라 비워진다. 따라서 껐다 켤 때마다 `npm run cvm:repin`(RTMR3 재핀 → smoke:live → 사이트 재빌드·재배포)을 돌리고 크리에이터가 복구 파일로 다시 provision 해야 한다. 심사 기간에는 켜 둔다.
...
- [x] **브라우저에서 실전 확인:** 크리에이터 앱을 `VITE_INDEXER_URL=<endpoint> VITE_EXPECTED_MEASUREMENT_HEX=<rtmr3>`로 띄워 실제 quote로 provision이 통과하는지. `creator/src/qvl-verifier.ts`는 아직 실제 quote를 본 적이 없다. 실패하면 이슈로 등록하고 `attest:inspect` 결과와 대조.
...
- [x] 저장소 public 전환 — 2026-09-13. 전환 전에 트리·전체 히스토리를 스캔(시드·주소·비공개 경로 0건)하고 git 히스토리를 다시 써서 모든 브랜치를 force-push 했다. **기존 clone은 rebase 하지 말고 새로 clone 할 것.** 시크릿 스캔은 릴리스마다 반복 (`npm run qa:secrets`, `git log -p | grep -i seed` 같은 수동 확인).
