...
- **No secrets in git, ever.** `.midnight-state.json` and wallet state are ignored. Never paste a seed, a viewing key, or a Lace address (a devnet address is also that wallet's mainnet address) into a tracked file.
