...
Blindfold는 이전에 만들었던 **Drop**(잠긴 콘텐츠를 프라이버시 결제로 여는 "눈 가린 우체부")을
...
Drop (previous prototype) sells unlockable content:
...
The earlier prototype relied on an in-band payment note: the only channel from an anonymous
buyer to the indexer. Midnight has no such note. It has something better for this job: a **Compact smart
...
- The previous project's design docs live in the private Drop repo (not on GitHub). Ask the owner. Do not copy its git history: it contains real keys.

## 4. Architecture (target)

The earlier prototype's four dependencies and their Midnight replacements:

| Earlier prototype | Blindfold on Midnight |
|---|---|
| A payment note carries `drop_id ‖ e_pub` | Circuit `purchase(dropId, ePub, coin)`; `ePub` is `disclose()`d into a ledger `Map<Uint<64>, Bytes<32>>` |
| Shielded coins hide the buyer | Zswap shielded NIGHT; the coin is `receiveShielded` into the contract |
| The indexer needs a wallet key to see payments | Not needed: the indexer reads the public `purchases` map through the Midnight indexer GraphQL |
| Payment QR scanned by a mobile wallet | DApp Connector v4 (`window.midnight.*`), `findDeployedContract(...).callTx.purchase(...)` |
