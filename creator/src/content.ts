// Content blob (interface I4) — AES-256-GCM with layout `nonce(12) || ciphertext || tag(16)`.
// `decryptContent` is the byte-for-byte counterpart of Lane C (creator/src/content.ts):
// C encrypts, B decrypts, both via WebCrypto. `encryptContent` is kept so tests can build
// fixtures that match exactly what C produces.

import { bytesToArrayBuffer, concatBytes, fromHex, sha256Hex } from "./bytes";
...
export function parseSha256Hex(value: string): Uint8Array {
  if (!/^[0-9a-fA-F]{64}$/.test(value)) {
    throw new Error("h_content must be a 32-byte sha256 hex value");
  }
