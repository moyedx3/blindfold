...
//
// The dispatch blob is a libsodium `crypto_box_seal(K_drop, e_pub)` (80 bytes) produced by the
// indexer; we open it with libsodium-wrappers — same Curve25519 sealed box, byte-compatible.
// `crypto_box_seal_open` THROWS on a MAC mismatch (a blob not meant for us)
...
/** libsodium sealed box — used by tests and the mock indexer. */
